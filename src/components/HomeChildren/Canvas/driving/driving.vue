    ...
    data() {
      return {
        ...
        driving: ['plate_num', 'vehicle_type', 'owner', 'addr',
          'use_character', 'model', 'vin', 'engine_num', 'register_date'
          , 'issue_date', 'plate_num_b', 'record', 'passengers_num', 'total_quality',
          'prepare_quality', 'overall_size'],
        result_list: [],
        pre: ['号牌号码：', '车辆类型：', '所有人：', '住址：',
          '使用性质：', '品牌型号：', '车辆识别代号：', '发动机号码：', '注册日期：', '发证日期：'
          ,'号牌号码：','档案编号：','核定载人数：','总质量：','整备质量：','外廓尺寸：'],
        loading: false,
        err: false
      }
    },
          ...
          .then(res => {
            ...
            if (data.hasOwnProperty("driving_result")) {
              ...
              if (result['type'] === 0) {
                ...
                for(let m=0;m<6;m++){
                  this.result_list[m].pre=this.pre[10+m];
                }
              }
            } else {
              this.result_list = []
